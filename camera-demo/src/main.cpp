...
void camera_switch(void) {
    ...
    pwm_set_enable(PWM_DEVICE_1, PWM_CHANNEL_1, enable);
}
#endif
...
static void io_init(void) {
    ...
    /* pwm IO.*/
    fpioa_set_function(LED_IR_PIN, fpioa_function_t(int(FUNC_TIMER0_TOGGLE2) + 1 * 4));
    /* Init PWM */
...
int main() {
    ...
    g_lcd_gram = (uint32_t*)malloc(320 * 240 * 2);
    ...
    dvp_set_display_addr((uintptr_t)g_lcd_gram);
    dvp_config_interrupt(DVP_CFG_START_INT_ENABLE | DVP_CFG_FINISH_INT_ENABLE, 0);
    dvp_disable_auto();
